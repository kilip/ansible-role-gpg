...
class GpgKey(object):
        ...
        for index, fk in enumerate(keyinfo["keys"]):
            ...
            if fk["trust_level"] in ['i', 'd', 'r', 'e']:
                self.module.fail_json(
                    msg="key is either expired or invalid [trust={}] [expiration={}]".format(fk["trust_level"],
                                                                                             fk["expirationdate"]))
    ...
    def run_fpr_present(self):
        ...
        if not installed:
            ...
            if keyinfo["keys"][0]["trust_level"] in ['i', 'd', 'r', 'e']:
                ...
                cmd += [fpr]
                rc, stdout, stderr = self.module.run_command(args=cmd, check_rc=True)
                self.module.fail_json(msg="key is either expired or invalid [trust={}] [expiration={}]".format(
                    keyinfo["keys"][0]["trust_level"], keyinfo["keys"][0]["expirationdate"]))
    ...
    def check_versions(self):
        ...
        self._vv(
            "gpg_key module requires at least gnupg version [{}] and libgcrypt version [{}]".format(versions['gpg'],
                                                                                                    versions[
                                                                                                        'libgcrypt']))
        ...
        if version.parse(versions['gpg']) < version.parse(req_gpg) or version.parse(
            ...
            self.module.fail_json(
                msg="gpg version [{}] and libgcrypt version [{}] are required; [{}] and [{}] given".format(req_gpg,
                                                                                                           req_libgcrypt,
                                                                                                           versions[
                                                                                                               'gpg'],
                                                                                                           versions[
            ...
            self._vv(
                "gnupg version [{}] and libgcrypt version [{}] detected".format(versions['gpg'], versions['libgcrypt']))
    ...
    def check_file(self):
        ...
        if not os.path.isfile(self.module.params["file"]):
            self.module.fail_json(
                msg="the keyfile [{}] does not exist on the target machine".format(self.module.params["file"]))
        else:
